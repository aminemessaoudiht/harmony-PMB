...
// +-------------------------------------------------+
// $Id: relance.class.php,v 1.10 2021/05/11 14:15:08 dgoron Exp $
...
class relance {
	...
	public static function get_instance_filter_list() {
		...
		$filter_list = new filter_list("empr","empr_list","b,n,c,cs,g","c".$localisation.",13,2,3".$filter_p_perso.($empr_filter_relance_rows?",".$empr_filter_relance_rows:""),"n,g");
		/*
	...
	<select class='readers_relances_ui_action' id='action_".$id_empr."' name='action_".$id_empr."'>
	";
    ...
    public static function filter_last_dates($liste_ids,$champ,$selected = array(),$sort=false) {
    	...
    	$requete="SELECT DISTINCT date_relance FROM pret WHERE printed > 0";
    	$res=pmb_mysql_query($requete);
    	if($res && pmb_mysql_num_rows($res)){
    		while ($ligne=pmb_mysql_fetch_object($res)) {
