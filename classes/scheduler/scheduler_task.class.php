...
// +-------------------------------------------------+
// $Id: scheduler_task.class.php,v 1.11 2020/01/21 10:15:24 dgoron Exp $
...
class scheduler_task {
	...
	public function update_progression($percent) {
		...
		if ($this->id_tache) {
			...
			$requete = "update taches set indicat_progress ='".round($percent)."', rapport='".htmlspecialchars(serialize($this->report), ENT_QUOTES,$charset)."' where id_tache=".$this->id_tache;
			pmb_mysql_query($requete);
		}
	}
