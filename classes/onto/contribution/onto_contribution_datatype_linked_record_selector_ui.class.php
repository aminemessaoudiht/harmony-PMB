...
// +-------------------------------------------------+
// $Id: onto_contribution_datatype_linked_record_selector_ui.class.php,v 1.8 2020/11/05 15:44:01 jlaurent Exp $
...
// 		$selector_url = $this->get_resource_selector_url($property->range[0]);
		...
		if (!empty($datas)) {
			...
			foreach($datas as $key=>$data){
				...
				$selector = notice_relations::get_selector('!!onto_row_id!![!!onto_row_order!!][assertions][relation_type]',(isset($formated_value['relation_type']) ? $formated_value['relation_type'] : ""));
			...
			$selector = notice_relations::get_selector('!!onto_row_id!![!!onto_row_order!!][assertions][relation_type]');
			$inside_row = str_replace('!!onto_row_content_linked_record_selector!!', $selector, $inside_row);
			$inside_row = str_replace("!!onto_row_content_marclist_range!!",$property->range[0] , $inside_row);
