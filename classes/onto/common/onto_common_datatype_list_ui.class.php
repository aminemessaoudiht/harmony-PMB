...
// +-------------------------------------------------+
// $Id: onto_common_datatype_list_ui.class.php,v 1.12 2021/02/03 14:58:59 qvarin Exp $
...
class onto_common_datatype_list_ui extends onto_common_datatype_ui {
	...
	public static function get_selector_form($item_uri, $property, $restrictions, $datas, $options_values) {
		...
		foreach($options_values as $id => $value){
			...
			$options.= '<option value="'.htmlentities($id, ENT_QUOTES, $charset).'" '.(in_array($id, $values) ? 'selected="selected"' : '').' '.$display_none.'>'.htmlentities($value, ENT_QUOTES, $charset).'</option>';
		}
