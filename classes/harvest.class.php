...
// +-------------------------------------------------+
// $Id: harvest.class.php,v 1.10 2021/01/29 09:34:18 dgoron Exp $
...
class harvest {
	...
	*/
	...
	public function get_form() {
		...
		if(!$this->id){
			$interface_form->set_label($msg['admin_harvest_build_form_add']);
		}else{
			$interface_form->set_label($msg['admin_harvest_build_form_edit']);
		}
		...
		$content_form=str_replace('!!src_list!!',$src_list,$content_form);
	...
	public function save() {
		if(!$this->id){ // Ajout
			$req="INSERT INTO harvest_profil SET 
				harvest_profil_name='".addslashes($this->info['name'])."'
			";	
			pmb_mysql_query($req);
...
class harvests {	
	...
	public function fetch_data() {
		...
		if (pmb_mysql_num_rows($resultat)) {
			while($r=pmb_mysql_fetch_object($resultat)){	
				$this->info[$i]= new harvest($r->id_harvest_profil);	
