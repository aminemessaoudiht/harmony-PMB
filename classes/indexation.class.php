...
// +-------------------------------------------------+
// $Id: indexation.class.php,v 1.66 2020/11/03 14:48:06 dgoron Exp $
				...
				if(!empty($table['TABLEFIELD'])){
    				for ($j=0;$j<count($table['TABLEFIELD']);$j++) {
    					...
    					if(isset($table['TABLEFIELD'][$j]['TRANSLATION'])){
    						...
    						$this->tab_code_champ[$v['ID']][$table['TABLEFIELD'][$j]["value"]]['translation']=$table['TABLEFIELD'][$j]['TRANSLATION'];
						...
						for($j=0 ; $j<count($callback_return) ; $j++) {
						    if (is_array($callback_return[$j])) {
    						    foreach ($callback_return[$j] as $callback_lang => $callback_value) {
        							$tab_mots = array();
        							$tab_field_insert[] = $this->get_tab_field_insert($object_id, $callable_data[$i], $order_fields, $callback_value, $callback_lang);
