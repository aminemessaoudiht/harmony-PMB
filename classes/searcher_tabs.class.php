...
// +-------------------------------------------------+
// $Id: searcher_tabs.class.php,v 1.87 2021/06/11 06:42:30 moble Exp $
...
class searcher_tabs {
	...
	protected function parse_search_file() {
	    ...
	    if(!$dejaParse){
		    ...
		    for ($i = 0; $i < count($selector_mode)-1; $i++) {
			...
			if($key_file){
				...
				fwrite($tmp,serialize(array(
						...
						$this->tabs,
				        $this->default_authorities_selector_mode
				)));
				fclose($tmp);
			}
    	}
    ...
    public function show_result() {
    	...
    	if(is_array($this->objects_ids) && count($this->objects_ids)) {
    		...
    		print searcher::get_search_back_button($this->base_url, 'AUT');
