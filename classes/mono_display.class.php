...
// +-------------------------------------------------+
// $Id: mono_display.class.php,v 1.335 2021/03/31 15:31:42 dgoron Exp $
	...
	public function do_header() {
		...
		if ($this->notice->niveau_biblio=='b') {
			...
			if (pmb_mysql_num_rows($execute_query)) {
    			...
    			$this->header.=" <i>".(empty($row->aff_date_date) ? sprintf($msg["bul_titre_perio"],$row->tit1):sprintf($msg["bul_titre_perio"],$row->tit1.", ".$row->num_bull." [".$row->aff_date_date."]"))."</i>";
    			$this->header_texte.=" ".(empty($row->aff_date_date) ? sprintf($msg["bul_titre_perio"],$row->tit1):sprintf($msg["bul_titre_perio"],$row->tit1.", ".$row->num_bull." [".$row->aff_date_date."]"));
			}
