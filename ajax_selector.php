...
// +-------------------------------------------------+
// $Id: ajax_selector.php,v 1.137 2021/05/18 12:27:44 dgoron Exp $
...
switch($completion):
	...
	case 'collections':
		...
		$requete="select if(collection_issn is not null and collection_issn!='',concat(collection_name,', ',collection_issn, ' (', ed_name, ')'),concat(collection_name, ' (', ed_name, ')')) as coll,collection_id from collections left join publishers on collections.collection_parent = publishers.ed_id where if(collection_issn is not null and collection_issn!='',concat(collection_name,', ',collection_issn),collection_name) like '".addslashes($start)."%' $restrict order by index_coll limit 20";
		$origine = "SQL" ;
		break;
	case 'subcollections':
