...
// +-------------------------------------------------+
// $Id: animation.class.php,v 1.4 2021/03/05 10:07:13 qvarin Exp $
...
class animation
{
    ...
    private function init(int $animation_id)
    {
        ...
        $this->errors = $this->animation_conf->check_conf();
        $calendar_data = $this->animation_conf->get_calendar_data();
        if (!empty($calendar_data) && !empty($calendar_data['type'])) {
            $this->add_parametre_perso($calendar_data['type']);
        } else {
            $this->errors[] = plugins::get_message('animation', "animation_error_article_num_type");
        }
     ...
     *
     * @param string|integer $num_type
     * @return boolean
     */
    private function check_parametre_perso($num_type)
    {
        if (empty($this->id_champ)) {
            $query = 'SELECT idchamp FROM cms_editorial_custom WHERE name = "cp_animation_id" and num_type = "' . addslashes($num_type) . '"';
     ...
     * @param string|integer $num_type
     */
    private function add_parametre_perso($num_type)
    {
        if (! $this->check_parametre_perso($num_type) && !empty($num_type)) {
            $query = 'INSERT INTO cms_editorial_custom set 
                        num_type = "' . addslashes($num_type) . '", name="cp_animation_id", 
                        ...
                        multiple="0", obligatoire="0", ordre="0", search="0", export="0", exclusion_obligatoire="0", opac_sort="100", comment="", custom_classement=""';
    ...
    public function animation_has_article()
    {
        if (empty($this->id_article)) {
            ...
            $query = 'SELECT id_article FROM cms_articles 
                        ...
                        WHERE article_num_type = "' . $calendar_data['type'] . '" AND 
                        cms_editorial_custom_integer = "' . addslashes($this->animation_id) . '" AND 
    ...
    private function get_animation_data()
    {
        if (empty($this->animation_data)) {
            $query = 'SELECT anim_animations.name, anim_animations.comment, anim_animations.description, anim_events.start_date, anim_events.end_date FROM anim_animations 
