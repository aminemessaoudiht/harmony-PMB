...
// +-------------------------------------------------+
// $Id: fields_empr.inc.php,v 1.250 2021/06/10 10:26:04 gneveu Exp $
...
function aff_url_empr($field,&$check_scripts){
	...
	foreach ($values as $value) {
		...
		$display_temp.= $msg['persofield_url_link']."<input id='".$field['NAME']."_link".$count."' type='text' class='saisie-30em' name='".$field['NAME']."[link][" . $count . "]' data-form-name='".$field['NAME']."_link' onchange='cp_chklnk_".$field["NAME"]."(".$count.",this);' value='".htmlentities($avalues[0],ENT_QUOTES,$charset)."'>";
		$display_temp.=" <input class=\"bouton\" type='button' value='".$msg['persofield_url_check']."' onclick='cp_chklnk_".$field["NAME"]."(".$count.",this);'>";
		//$display_temp.="<br />";
		$display_temp.="&nbsp;".$msg['persofield_url_linklabel']."<input id='".$field['NAME']."_linkname".$count."' type='text' class='saisie-15em' size='".$options['SIZE'][0]['value']."' name='".$field['NAME']."[linkname][" . $count . "]' data-form-name='".$field['NAME']."_linkname' value='".htmlentities($avalues[1],ENT_QUOTES,$charset)."'>";
		$target_checked = 'checked="checked"';
		if (isset($avalues[2]) && ($avalues[2] == 0)) {
			$target_checked = '';
		}
		$display_temp.="&nbsp;<input id='".$field['NAME']."_linktarget".$count."' type='checkbox' name='".$field['NAME']."[linktarget][" . $count . "]' data-form-name='".$field['NAME']."_linktarget' value='1' ".$target_checked."><label for='".$field['NAME']."_linktarget".$count."'>&nbsp;".$msg['persofield_url_linktarget']."</label>&nbsp;";
		if(isset($options['REPEATABLE'][0]['value']) && $options['REPEATABLE'][0]['value']) {
			$button_add = '';
			if (end($values) == $value) {
