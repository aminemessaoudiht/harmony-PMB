...
// +-------------------------------------------------+
// $Id: search.tpl.php,v 1.54 2021/02/15 08:01:00 dgoron Exp $
...
if($mode==7 || $mode==8 || $current_module=="circ") $search_form.=	"
	<div class='row'>
		<div class='left'>
			<input type='submit' class='bouton' value='".htmlentities($msg["142"], ENT_QUOTES, $charset)."' id='search_form_submit'/>
			".(in_array($current_module, array("circ", "catalog")) ? ($categ=='consult' ? "" : "<input type='button' class='bouton' id='save_predefined_search' value='".htmlentities($msg["search_perso_save"], ENT_QUOTES, $charset)."' onClick=\"enable_operators();this.form.launch_search.value=1; this.form.action='!!memo_url!!'; this.form.page.value=''; !!target_js!! this.form.submit()\"/>") : "")."
		</div>
	</div>";
