								...
								<td colspan="3">
									<template v-if="!formdata.animation.hasChildrens">
										<label class='etiquette'> {{ pmb.getMessage("animation", "animation_type") }} </label>
					 					<select :id="'person_' + indexPerson" v-model="person.numPrice" @change="getcustomField(indexPerson)">
	...
	export default {
		...
		methods : {
		    ...
		    isValidPersons: function () {
		        ...
		        if (this.registrationlist.nbRegistredPersons == 0) {
		            return false;
		        }
			...
			deletePerson : function(index) {
				...
				this.registrationlist.nbRegistredPersons--;
			},
			...
			checkAvailablePlace : function() {
				// Dans le cas des quotas illimités
				if (this.formdata.animation.allQuotas.animationQuotas.global == 0) {
					return true;
				}
				if (this.formdata.animation.allQuotas.availableQuotas.global - this.registrationlist.registrationListPerson.length > 0) {
					return true;
				}
				return false;
			},
			...
			getcustomFieldAnimation : function(indexPerson, idAnimation) {
			    if (this.formdata.animationsSelected && this.formdata.animationsSelected.length) {
			        var person = this.registrationlist.registrationListPerson[indexPerson];
			...
			isValidPhone : function() {
				var tempPhone = this.registrationlist.phoneNumber.replace(/[\W\s]/gm, '');
				if (isNaN(tempPhone)) {
					return false;
				}
				return true;
