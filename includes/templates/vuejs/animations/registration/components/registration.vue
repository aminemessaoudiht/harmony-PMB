<template>
	<div id="list">
		<h3 v-if="!numanimation">{{ pmb.getMessage("animation", "list_registration_title") }}</h3>
		<div class="row">
			<span>{{ pmb.getMessage("animation", "animation_search_participant_reservation") }}</span>
			<input id="search" v-model="search" type="text"/>
		</div>
		<div v-if="!numanimation" class="row">
			<div class="row">
				<select v-model="animSelected">
					<option value="0" selected>{{ pmb.getMessage("animation", "animation_all_animations") }}</option>
			...
			<div class="row">
				<select v-model="statusSelected">
					<option value="0" selected>{{ pmb.getMessage("animation", "animation_all_registration_status") }}</option>
		...
		<table>
			<tr>
				...
				<th style="cursor:pointer;" @click="setSort('name')" >{{ pmb.getMessage("animation", "list_registration_name") }} <i :class="getClassSort('name')"></i></th>
				<th style="cursor:pointer;" @click="setSort('email')">{{ pmb.getMessage("animation", "list_registration_email") }}<i :class="getClassSort('email')"></i></th>
				<th>{{ pmb.getMessage("animation", "list_registration_phone") }}</th>
 				<th style="cursor:pointer;" @click="setSort('nbRegistredPersons')">{{ pmb.getMessage("animation", "registration_nb_persons") }} <i :class="getClassSort('nbRegistredPersons')"></i></th>
				<th v-if="!numanimation">{{ pmb.getMessage("animation", "animations") }}</th>
				<th style="cursor:pointer;" @click="setSort('rawDate')">{{ pmb.getMessage("animation", "incription_date") }} <i :class="getClassSort('rawDate')"></i></th>
			...
			<template v-if="filteredRegistration.length > 0">
				<template v-for="(registration, index) in filteredRegistration">
					<tr v-if="(registration.animation.id ==  animSelected || 0 == animSelected) && (0 == statusSelected || registration.numRegistrationStatus == statusSelected)" 
						:key='index'
						...
						:class="[ index % 2 == 0 ? 'odd' : 'even', index == hover ? 'surbrillance' : '' ]">
							...
							<td>{{ registration.nbRegistredPersons }}</td>
							<td v-if="!numanimation">{{ registration.animation.name }}</td>
							<td>
								<template v-if="registration.date">{{ registration.date }}</template>
							</td>
							<td>
								<template v-if="registration.registrationStatus.name">{{ registration.registrationStatus.name }}</template>
							</td>
			...
			<template v-else>
				<tr>
					<td colspan="8" style='text-align: center'>{{ pmb.getMessage("animation", "list_registration_empty") }}</td>
				</tr>
			</template>
		</table>
		<div class="row">
			<div class="left">
				<input @click="validatedRegistrationList" class="bouton" type="button" :value="pmb.getMessage('animation', 'incription_validated_list')"/>
			</div>
			<div class="right">
				<input @click="delRegistration" class="bouton" type="button" :value="pmb.getMessage('animation', 'incription_delete_list')"/>
			</div>
		</div>
	</div>
</template>
...
<script>
	export default {
		props : ["registrationlist", 'animationlist', "pmb", "numanimation", "statuslist", "selectedstatus"],
		data: function () {
		    return {
		        animSelected : 0,
		        statusSelected : 0,
		...
		computed : {
			filteredRegistration : function() {
				...
				return d.filter(function(registration) {
				    var map = {
				        '' : ' |-|_|\'|"|\\.|:|;',
				        'a' : 'á|à|ã|â',
				        'e' : 'é|è|ê',
					...
					let regist = registration.name.toLowerCase();
				    for (var pattern in map) {
				    	regist = regist.replace(new RegExp(map[pattern], 'g'), pattern);
				    }
					...
					regist = registration.email.toLowerCase();
				    for (var pattern in map) {
				    	regist = regist.replace(new RegExp(map[pattern], 'g'), pattern);
				    }
					...
					regist = registration.phoneNumber.toLowerCase();
				    for (var pattern in map) {
				    	regist = regist.replace(new RegExp(map[pattern], 'g'), pattern);
				    }
		...
		methods: {
			...
			checkValidation : function(index) {
				if (!this.registrationlist[index].validated && this.registrationlist[index].animation.allQuotas.availableQuotas.global >= this.registrationlist[index].nbRegistredPersons) {
					return true;
				}
				return false;
			},
