...
// +-------------------------------------------------+
// $Id: resa_func.inc.php,v 1.158 2020/12/18 09:14:10 dgoron Exp $
...
function resa_list ($idnotice=0, $idbulletin=0, $idempr=0, $order="", $where = "", $info_gestion=NO_INFO_GESTION, $url_gestion="",$ancre="") {
	...
	while ($data = pmb_mysql_fetch_array($req)) {
		...
		$resa_situation->set_resa($resa)
				...
				->set_precedenteresa_idnotice($precedenteresa_idnotice)
				->set_precedenteresa_idbulletin($precedenteresa_idbulletin)
				->set_rank($rank)
				->set_no_aff($no_aff)
		...
		$precedenteresa_idnotice = $resa_situation->get_precedenteresa_idnotice();
		$precedenteresa_idbulletin = $resa_situation->get_precedenteresa_idbulletin();
