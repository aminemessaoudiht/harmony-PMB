...
// +-------------------------------------------------+
// $Id: search.class.php,v 1.361 2021/05/31 14:30:19 gneveu Exp $
...
// 		else $r.="<select name='add_field' id='add_field'>\n";
		...
		foreach ($this->list_criteria as $group=>$criteria) {
		    $r .= "<option class='select-group' disabled value='' class='optgroup_multicriteria' style='color:#000; font-weight:bold;'>".htmlentities($group,ENT_QUOTES,$charset)."</option>\n";
				...
				if ((string)$i!=$delete_field) {
					...
					if (($i==(count($search)-1))||(($delete_field==(count($search)-1))&&($i==(count($search)-2)))) {
						...
						if ($no_search) {
    					    ...
    					    $r.="<span class='search_submit'><input type='submit' id='search_form_submit' class='boutonrechercher' value='".$msg["142"]."'/></span>";
						}
					    $r.="\n</td></tr>";
					}
					$n++;
