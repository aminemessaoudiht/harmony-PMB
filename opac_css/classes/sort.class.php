...
// +-------------------------------------------------+
// $Id: sort.class.php,v 1.110 2021/03/04 16:10:07 dbellamy Exp $
...
class sort {
	...
	public static function show_tris_selector($sort_name = "notices") {
		...
		if ($sortArray[0] == 0) {
			...
			if (!is_object($sort) && $sort !== "") {
			    $sel_sort = $sort;
			} elseif (!empty($_SESSION["last_sort$sort_name"])) {
	...
	public static function show_tris_in_result_list($nbr_lignes, $sort_name="notices") {
		...
		if ($nbr_lignes<=$opac_nb_max_tri) {
			...
			$end_html = '';
			if(!empty($_SESSION["last_sort$sort_name"]) || $opac_default_sort_display) { //Encapsulation des éléments de tri dans un container pour faciliter le style
