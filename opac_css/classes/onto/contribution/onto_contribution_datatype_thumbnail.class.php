...
// +-------------------------------------------------+
// $Id: onto_contribution_datatype_thumbnail.class.php,v 1.6 2021/04/21 12:43:29 gneveu Exp $
...
class onto_contribution_datatype_thumbnail extends onto_common_datatype_file {
	...
	public function get_value($all = false) {
	    if ($all && $this->value) {
	        if (is_object(json_decode($this->value))) {
	            $value = json_decode($this->value);
	            $folder = new upload_folder($value->id_upload_directory);
	            $thumbnail_path = $folder->repertoire_path.$value->path;
	            $value->thumbnail = file_get_contents($thumbnail_path);
	        }
	        return $value;
	    } 
