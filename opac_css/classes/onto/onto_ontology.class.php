...
// +-------------------------------------------------+
// $Id: onto_ontology.class.php,v 1.25 2021/02/03 14:58:59 qvarin Exp $
	...
	public function get_properties( ) {
		if (!$this->properties_uri) {
			$query  = "select * where {
				...
				optional {
					?property pmb:form_uri ?form_uri
				} .
				optional {
					?property pmb:form_id ?form_id
				} .
				optional {
					?property pmb:nosearch ?no_search
				}
			}";
			if($this->store->query($query)){
				if($this->store->num_rows()){
...
// 							}
						...
						if (!empty($elem->form_uri)) {
						    $this->properties_uri[$elem->property]->form_uri = $elem->form_uri;
						}
						if (!empty($elem->form_id)) {
						    $this->properties_uri[$elem->property]->form_id = $elem->form_id;
						}
						if (!empty($elem->no_search)) {
						    $this->properties_uri[$elem->property]->no_search = $elem->no_search;
						}
					}
				}
