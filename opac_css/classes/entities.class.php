...
// +-------------------------------------------------+
// $Id: entities.class.php,v 1.7.2.1 2021/06/11 10:32:27 qvarin Exp $
...
class entities {
    ...
    public static function get_picto_url_from_entity(string $entity, int $id) {
        ...
        switch ($entity) {
            ...
            case 'titre_uniforme':
            case 'titres_uniformes':
                $work = new titre_uniforme($id);
                if (!empty($work->get_nature()) && !empty($work->get_type())) {
                    $picto_url = get_url_icon('authorities/tu_'.$work->get_nature().'_'.$work->get_type().'_icon.png');
                    if (file_exists($picto_url)) {
                        return $picto_url;
                    }
                }
            ...
            case 'auteurs' :
                ...
                if (!empty($author->type)) {
                    $picto_url = get_url_icon("authorities/author_".$author->type."_icon.png");
                    if (file_exists($picto_url)) {
                        return $picto_url;
                    }
                }
                return get_url_icon('authorities/author_icon.png');
            case 'docnum':
                return get_url_icon("icone_nouveautes.png");
            case 'authperso':
                return get_url_icon('authorities/authperso_icon.png');
            default:
                $picto_url = get_url_icon('authorities/'.$entity.'_icon.png');
                if (file_exists($picto_url)) {
                    return $picto_url;
                }
                break;
