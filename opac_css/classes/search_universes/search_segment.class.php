...
// +-------------------------------------------------+
// $Id: search_segment.class.php,v 1.25 2021/01/20 09:41:17 dgoron Exp $
...
// 				search_segment_facets::set_num_segment($this->id);
	...
	public function get_form($ajax = false) {
	    ...
	    $html = str_replace('!!last_query!!', $universe->get_universe_query(), $html);
	...
	public function get_parent_universe_data() {
		...
		if ($this->num_universe != '') {
			...
			$universe = new search_universe($universe_data->id_search_universe);
			$html = $search_segment_parent_universe;
			$html = str_replace('!!segment_universe_label!!', htmlentities($universe_data->search_universe_label, ENT_QUOTES, $charset), $html);
			$html = str_replace('!!segment_universe_description!!', htmlentities($universe_data->search_universe_description, ENT_QUOTES, $charset), $html);
			$html = str_replace('!!segment_universe_id!!', $this->num_universe, $html);
	...
	public function get_display_search() {
		...
		$display = str_replace("!!search_segment!!", search_segment_search_view::get_display_search(), $display);
		...
		$display = str_replace("!!search_segment_result!!", $this->get_display_results($display_navbar, $display_sort_selector), $display);
		return $display;
	}
	...
	public function get_nb_results($ajax_mode = false) {
	    ...
	    return $this->search_result->get_nb_results($ajax_mode);
	}
