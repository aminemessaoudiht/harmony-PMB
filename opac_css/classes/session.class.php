...
// +-------------------------------------------------+
// $Id: session.class.php,v 1.1.2.2 2021/06/11 10:32:27 qvarin Exp $
    ...
    private function rec_nav_history()
    {
        ...
        if (! empty($this->data["nav_item"])) {
            ...
            if (isset($this->data["nav_item"]["date"])) {
                ...
                $this->data["nav_item"]["date"] = date($msg["date_format"], $this->data["nav_item"]["date"] / 1000);
            }
                ...
                if (! empty($parse_str["lvl"])) {
                    ...
                    switch ($parse_str["lvl"]) {
                        case "search_universe":
                            ...
                            $this->data["nav_item"]["title"] = search_universe::get_label_from_id($id);
                            break;

                        case "search_segment":
                            $id = intval($parse_str["id"]);
                            $this->data["nav_item"]["segment_id"] = $id;
                            $this->data["nav_item"]["title"] = search_segment::get_label_from_id($id);
                            break;
            ...
            $title = str_replace(array(
                $msg['opac_title'],
                strtolower($msg['opac_title'])
            ), '', $this->data["nav_item"]["title"]);
            $title = trim(html_entity_decode($title, ENT_QUOTES, $charset));
            $this->data["nav_item"]["title"] = htmlspecialchars(strip_tags($title), null, $charset);
