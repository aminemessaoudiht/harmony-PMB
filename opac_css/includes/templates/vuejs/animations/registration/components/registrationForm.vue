	...
	export default {
		...
		computed: {
			can_add_contact: function () {
				...
				if (this.registration.barcode != "" && this.registration.nbRegistredPersons != "" && this.registration.nbRegistredPersons != 0) {
		...
		methods : {
				    ...
				    if (contact) {
					    if (!contact.barcode && !this.registration.barcode) {
					        if (contact.name != this.registration.name) {
						        // L'utilisateur à modifier le nom
						        return true;
						    }
					    } else {
						    if (contact.barcode != this.registration.barcode) {
						    	// code barre différents ce n'est plus la personne de contact
						        return true;
						    }
						    if (contact.barcode == this.registration.barcode && contact.name != this.registration.name) {
						        // L'utilisateur à modifier le nom (on fait rien)
						        return false;
						    }
					    }
				    }
			...
			addPerson: function () {
				...
				// Si la liste d'attente est activée, on impose aucune limite
				...
				// On verifie s'il reste des places disponibles sur les animations selectionnées
			...
			deletePerson : function(index) {
				...
				this.registration.nbRegistredPersons--;
			},
			...
			isValidPersons : function() {
				...
				if (this.registration.nbRegistredPersons != "" && this.registration.nbRegistredPersons != 0) {
					for (let personIndex in this.registration.registrationListPerson) {
					    ...
					    if (this.animationsSelected && this.animationsSelected.length) {
							// inscription multiple, on vérifie pour chaque animations
			...
			save: function (event) {
				...
				if (this.isValidContact() && this.isValidPersons()) {
				  ...
				  //on verifie s'il reste des places disponibles sur les animation selectionner
					...
					// On vérifie s'il y a eu un problème lors du JSON.stringify ?
			...
			checkQuotas : function(){
				//Si on a pas limité les inscriptions, on continue
			...
			barcodeNotAlreadyUse : function(indexPerson) {
			    if (this.registration.nbRegistredPersons != "" && this.registration.nbRegistredPersons != 0 && this.registration.registrationListPerson[indexPerson].barcode != "") {
