...
// +-------------------------------------------------+
// $Id: navbar.inc.php,v 1.28 2021/06/11 08:49:40 arenou Exp $
...
function printnavbar_onclick($page, $nbrpages, $url,$action='') {
	...
	// page courante
	if ($nbrpages > 1) {
		$print .= $msg['page']." <input type='text' class='numero_page' name='page' id='navbar_input_page' value='$page' size='".strlen("$nbrpages")."'>/$nbrpages\n";
	}else {
		$print .= $msg['page']." $page/$nbrpages\n";
	}
	// next
	if ($suivante <= $nbrpages) {
