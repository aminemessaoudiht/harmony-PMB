...
// +-------------------------------------------------+
// $Id: user_update.inc.php,v 1.52 2021/02/23 08:48:54 jlaurent Exp $
...
if(isset($form_animations) && $form_animations) 		$droits = $droits + ANIMATION_AUTH;
	...
	if(!$id) {
		if(!empty($form_login) && $form_pwd==$form_pwd2) {
			if(!empty($duplicate_from_userid)) {
				...
				$requete.= "nb_per_page_gestion, user_email, user_alert_resamail, user_alert_contribmail, user_alert_demandesmail, user_alert_subscribemail, user_alert_suggmail, user_alert_serialcircmail, explr_invisible, explr_visible_mod, explr_visible_unmod, deflt_notice_replace_keep_categories";
				if (isset($sel_group)) {
					$requete.= ", grp_num";
				}
				$requete.= ") VALUES";
