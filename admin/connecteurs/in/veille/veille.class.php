...
// +-------------------------------------------------+
// $Id: veille.class.php,v 1.10 2021/05/10 14:29:06 jlaurent Exp $
...
class veille extends connector {
    ...
    public function rec_record($record, $source_id, $source_label) {
        ...
        //Dans le cas ou le record est présent dans l'entrepot mais à été passé en supprimé dans les veilles
        if ($record['item_status'] == '2') return;
        ...
        $order = $this->insert_categories_into_entrepot($source_id, $record, $date_import, $recid);
        $this->insert_concept_into_entrepot($source_id, $record, $date_import, $recid, $order);
        ...
        foreach($search_query as $amterm) {
            switch ($amterm->ufield) {
                ...
                case '610':
                    ...
                    $querySupp = "SELECT num_item FROM docwatch_items_tags WHERE num_tag IN IN (".implode(',',$ids).") ";
                    $result = pmb_mysql_query($querySupp);
                    if(pmb_mysql_num_rows($result)){
                        while ($row = pmb_mysql_fetch_assoc($result)) {
                ...
                case '60X':
                    ...
                    if (count($ids_categ)) {
                        $querySupp = "SELECT num_item FROM docwatch_items_descriptors WHERE num_noeud IN (".implode(',',$ids_categ).") ";
                        $result = pmb_mysql_query($querySupp);
                        if(pmb_mysql_num_rows($result)){
                            while ($row = pmb_mysql_fetch_assoc($result)) {
                    ...
                    // On recupere les concepts lies aux items
                    $querySupp = "SELECT id_item FROM skos_fields_global_index WHERE value LIKE '%". rawurlencode($amterm->values[0]) ."%'";
                    $result = pmb_mysql_query($querySupp);
                    if(pmb_mysql_num_rows($result)){
                        while ($row = pmb_mysql_fetch_assoc($result)) {
                            $ids_item[] = $row['id_item'];
                        }
                    }
                    
                    if (count($ids_item)) {
                        $querySupp = "SELECT num_object FROM index_concept WHERE num_concept IN (".implode(',',$ids_item).") ";
                        $result = pmb_mysql_query($querySupp);
                        if(pmb_mysql_num_rows($result)){
                            while ($row = pmb_mysql_fetch_assoc($result)) {
                                if (!in_array($row['num_object'], $ids_to_save)) {
                                    $ids_to_save[] = $row['num_object'];
                                }
                            }
                        }
                    }
        ...
        if (count($num_tag)){
            ...
            if(pmb_mysql_num_rows($result)){
                while ($row = pmb_mysql_fetch_assoc($result)) {
                    $value = $row['tag_title'];
                    $val=$this->checkArray($value);
                    $this->insert_content_into_entrepot($source_id, $record['item_url'], $date_import, $ufield, $usubfield, $field_order, 0, $val, $recid);
                    $field_order++;
                }
            }
        }
    ...
    private function insert_concept_into_entrepot($source_id, $record, $date_import, $recid, $order){
        $num_concept = array();
        
        $querySupp = "SELECT num_concept FROM index_concept WHERE num_object = " . $record['id_item'];
        $result = pmb_mysql_query($querySupp);
        if(pmb_mysql_num_rows($result)){
            while ($row = pmb_mysql_fetch_assoc($result)) {
                $num_concept[] = $row['num_concept'];
            }
        }
        
        if (count($num_concept)){
            $ufield="606";
            $usubfield="a";
            $query = 'SELECT value FROM skos_fields_global_index WHERE id_item IN ('.implode(',',$num_concept) .')';
            $result = pmb_mysql_query($query);
            if(pmb_mysql_num_rows($result)){
                while ($row = pmb_mysql_fetch_assoc($result)) {
                    $value = $row['value'];
                    $val = $this->checkArray($value);
                    $this->insert_content_into_entrepot($source_id, $record['item_url'], $date_import, $ufield, $usubfield, $order, 0, $val, $recid);
                    $order++;
                }
            }
        }
